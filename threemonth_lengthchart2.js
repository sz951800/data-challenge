...
d3.json("threemonth2.json").then(function (data) {
    ...
    const svg = d3.select("#line-chart-container-three2")
        .append("svg")
        .attr("width", width)
        .attr("height", height);
    ...
    svg.append("text")
        ...
        .attr("y", margin.top / 2)
        .style("text-anchor", "middle")
        .text("Trailing 3-month rent growth Over Time");
    ...
    .on("mouseover", function(d) {
        const [mouseX, mouseY] = d3.mouse(this.parentNode); // Get mouse coordinates relative to the parent SVG
        d3.select("#tooltip-three2")
            .style("visibility", "visible")
            .html("Month: " + d3.timeFormat("%Y-%m")(d.date) + "<br>Value: " + d.value)
            .style("top", (mouseY - 30) + "px") // Adjust the top position based on mouseY
            .style("left", (mouseX + 10) + "px"); // Adjust the left position based on mouseX
    })
    .on("mouseout", function() {
        d3.select("#tooltip-three2").style("visibility", "hidden");
    });
});
